...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
	  <h1 class="page-header" style="text-align:right">Country Detail</h1><br/>

      <?php
     			foreach($countries as $things=> $value)
  			{
  		?>
          <h2><?php echo 'Country Name : ' . $value['name'] . ' (' . $value['id'] . ')';?></h2><br/>
          ...
          <p><?php echo 'Admin Region : ' . $value['admin_region']; ?></p>
          <p><?php echo 'Income Level : ' . $value['income_level']; ?></p>
          <p><?php echo 'Lending Type : ' . $value['lending_type']; ?></p>
      <?php
  			}
  		?>
      <br/>
      <div class="navbar-form">
          <a href="<?php echo site_url('service/viewGDP/'.$value['id'])?>" class="btn btn-primary">Check for GDP</a>&nbsp&nbsp&nbsp
          <a href="<?php echo site_url('service/viewGNI/'.$value['id'])?>" class="btn btn-primary">Check for GNI</a>&nbsp&nbsp&nbsp
